...
- A heartbeat reuses one exact local Codex task. A standalone cron runs projectless or in one exact
  local project; this workflow does not provision worktree environments.
...
- For cron tasks, read current `model` and `model_reasoning_effort` defaults from the effective Codex
  `$CODEX_HOME/config.toml` when an entry omits them. Do not guess missing defaults.
- Omitted cron model settings copy defaults at sync time. The native tool currently rejects null and
  omitted values, so it cannot clear overrides for runtime inheritance.
- Heartbeats use their target task's model, effort, and project; do not send cron-only settings.
- The saved reader supports v1 thread heartbeats and local projectless cron tasks. Stop before mutation on unknown
...
- **Task definition:** name, enabled state, structured schedule, exactly one prompt source, an
  optional reusable preflight file, `kind: heartbeat` or `kind: cron` (legacy default), notification,
  and cron-only model, reasoning, or local-project overrides.
...
3. Resolve desired runtime values:
   - Preserve each existing heartbeat's `targetThreadId`. For a missing heartbeat, use native exact
     task reads to resolve its intended current-host task, and supply a `threadTargets` map from
     manifest ids to task ids. Keep host-specific ids out of the shared manifest. Never silently bind
     a missing automation to the dispatcher. Restore an archived target only when explicitly
     authorized; do not create a new task unless requested. Verify it is unarchived before scheduling.
   - A kind change requires a separately reviewed migration; do not silently replace an automation
     or its target. Missing target bindings produce plan blockers, not executable create actions.
   - Compile `minutely`, `hourly`, `daily`, `weekly`, or `monthly` schedules to deterministic RRULEs.
   - For cron only, use explicit `model` and `reasoning` values when present; otherwise use the
     current effective Codex defaults. Heartbeat model changes are outside automation sync.
   - For cron, treat no `local-project` as projectless. For a declared absolute path, call
     `list_projects` and require exactly one local project with that normalized path; record its `projectId`.
...
5. Use the emitted plan. For explicit planning inputs, start the bundled `plan` command and write one JSON object to its standard
   input through the active execution session. Include `actualTasks`, `projects`, optional
   `threadTargets`, and `defaults: {model, reasoningEffort}`. Do not place automation prompts in shell arguments or a
   pipeline.
...
6. For mutating reconciliation, present the exact ordered actions, affected ids and names, changed
   fields, heartbeat target ids, and digest. Resolve every plan blocker before requesting approval.
   Explain that deletes apply only to marked tasks absent from the manifest.
...
7. Apply the approved actions sequentially with `automation_update`:
   - heartbeat writes use `kind: heartbeat`, `destination: thread`, and the exact `targetThreadId`;
     cron writes use the planned local environment, project, model, and reasoning settings;
