...
- Do not use this skill alone when the task is purely styling or purely Typescript configuration.

## Relationship to Other Skills

- Assume `tanaab-coding-core` is active.
- Pair with `tanaab-typescript` for Vue + Typescript codebases.
...
4. Pull from `tanaab-typescript`, `tanaab-css`, or `tanaab-templates` when the task crosses those boundaries.
...
- Confirm cross-skill handoffs are explicit when Typescript, CSS, or templates are involved.
