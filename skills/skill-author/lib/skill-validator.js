...
function hasPirobasedPrefix(value) {
  return /^pirobased\s+/i.test(String(value ?? '').trim());
}
...
function validateFrontmatter({ frontmatter, requestedType, errors, warnings }) {
  ...
  if (frontmatter.description && !hasPirobasedPrefix(frontmatter.description)) {
...
async function validateSkillMarkdown({ actualType, errors, skillContent, skillPath, warnings }) {
  ...
  if (
    ...
    errors.push(
      `\`${actualType}\` skills must use the section order defined by the canonical ${actualType} template owned by piro-skill-author.`,
    );
  }
...
async function validateFolderName({ folderName, frontmatterName, skillPath, errors }) {
  ...
  if (pluginRoot) {
    if (folderName.startsWith(CANON_SKILL_MACHINE_PREFIX_WITH_HYPHEN)) {
      errors.push(
        `Plugin-contained skill folders must omit the machine prefix \`${CANON_SKILL_MACHINE_PREFIX_WITH_HYPHEN}\`: expected \`${stripSkillPrefix(frontmatterName || folderName)}\`.`,
      );
    }
  } else if (!folderName.startsWith(CANON_SKILL_MACHINE_PREFIX_WITH_HYPHEN)) {
    errors.push(
  ...
  if (frontmatterName) {
    const expectedFolderName = pluginRoot ? stripSkillPrefix(frontmatterName) : frontmatterName;
...
async function validateOpenAiMetadata({
  ...
  if (interfaceValues.short_description && !hasPirobasedPrefix(interfaceValues.short_description)) {
    errors.push(
...
function buildManualChecks({ expectedType }) {
  ...
  if (expectedType === 'coding') {
    checks.push(
      ...
      'Check that a retained `Deployment` section describes one canonical surface-local delivery mechanism and routes independent workflow topology to the workflow owner.',
      'Check that `GitHub Actions` maps owned lifecycle sections to canonical workflow templates without duplicating doctrine or embedding complete workflow YAML.',
