...
describe('skills/me-readiness/scripts/check-machine', () => {
  ...
  it('should not emit removed 1Password Environment readiness checks', async () => {
    const report = await runCheck({
      existingPaths: [
        '/Applications/1Password.app',
        '/Applications/Tailscale.app',
    ...
    const emittedCheckIds = new Set(report.checks.map((check) => check.id));

    for (const checkId of removedCheckIds) {
      assert.equal(emittedCheckIds.has(checkId), false, checkId);
    }
  });
