...
released-version fetch behavior for both `me` and `tanaab`. It verifies that the wrapper can clone
...
# should run boot.sh successfully using the default ssh me and tanaab sources
boot.sh \
  --op-token "$OPTOKEN" \
...
# should run boot.sh successfully using released me and tanaab sources and replace the existing checkouts
boot.sh \
  --op-token "$OPTOKEN" \
  --ssh-key 'omfsw2uztmi2xqpid5g3kiv6ba/id_test' \
  --me v1.0.0-beta.4 \
