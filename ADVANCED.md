...
The shared Codex default is `gpt-6-astra` with `xhigh` reasoning. Apply it through `ai:sync`, then
verify the generated configuration and a new task without a model override. Sync regenerates all
shared settings, so review differences from the live configuration before applying it. Existing
task model selections are independent of this default.

The shipped `AUTOMATIONS.yaml` entries omit model and reasoning overrides. `$piro-automation check`
uses the effective Codex defaults to identify drift; apply its reviewed sync plan to update saved
automations. Changing the shared file alone does not update their saved model selections.

For pirog's local OpenClaw environment, the intended primary model is `openai/gpt-6-astra` through
the existing OpenAI provider route. OpenClaw configuration is machine-owned and is not generated by
`ai:sync`. Before changing it, verify installed-version support and authenticated account access,
add Astra to any explicit model allowlist, and preserve working fallbacks. If no fallbacks exist,
verify the previous primary works before retaining it as a fallback. Validate configuration and
confirm a fresh session resolves Astra; handle any existing session override separately. Track a
required OpenClaw upgrade as a prerequisite instead of bundling it into the default-model change.
Keep provider authentication and private runtime configuration outside the repository.
