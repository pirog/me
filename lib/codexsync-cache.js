 ...
 * @param {Function} [statPath] Injectable lstat probe.
 * @returns {Promise<boolean>} Whether the path currently exists.
 * @throws {Error} When metadata access fails for a reason other than a missing path.
 */
export async function pathExists(targetPath, statPath = lstat) {
  try {
 ...
 * @throws {Error} When an entry cannot be inspected, except for a confirmed missing path.
 */
export async function collectManagedEntries(rootDir, entryMap = new Map(), statPath = lstat) {
  for (const managedPath of MANAGED_PATHS) {
    await collectEntry(rootDir, managedPath, entryMap, statPath);
  }

  return entryMap;
}

async function collectEntry(rootDir, relativePath, entryMap, statPath) {
  ...
  if (stats.isDirectory()) {
    ...
    for (const dirent of dirents.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      await collectEntry(rootDir, path.join(relativePath, dirent.name), entryMap, statPath);
    }
...
export async function syncEntries({ sourceEntries, sourceRoot, targetEntries, targetRoot }) {
  ...
  const pathsToSync = new Set([...diff.changed, ...diff.missing]);
  ...
  for (const [relativePath, sourceEntry] of sortedEntries) {
    if (!pathsToSync.has(relativePath)) {
      continue;
    }
    ...
    if (sourceEntry.type === 'dir') {
      if (diff.changed.includes(relativePath)) {
