...
async function brewEnvironment() {
  ...
  if ((await exists(AGENTBOX_HEALTH)) && (await exists(AGENTBOX_PLIST))) {
    skips.add('1password');
...
async function checkBrewfile({ root }) {
  ...
  const homebrewBun = await realpath(path.join(bunPrefix, 'bin', 'bun'));
  if (
    (await realpath(process.execPath)) !== homebrewBun ||
    globalThis.Bun.version !== expectedBun
  ) {
    return false;
  }
...
async function applyBrewfile({ root }) {
  ...
  const expectedBun = (await readFile(path.join(root, '.bun-version'), 'utf8')).trim();
  if (globalThis.Bun.version !== expectedBun) {
...
async function verifiedPluginSource(source, repo) {
  ...
  if (!expected.includes(origin)) throw new BlockedError(`Unexpected origin for ${repo}.`);
}
...
async function buildCurrent(source) {
  ...
  const stamp = path.join(source, 'node_modules', '.cache', 'me-profile-build.sha256');
...
export async function runProfileSetup(argv, options = {}) {
