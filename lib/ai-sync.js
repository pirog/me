...
export function buildAiSyncEnvironment({
  env = process.env,
  homedir = os.homedir(),
  repoRoot = REPO_ROOT,
} = {}) {
...
export function buildAiSyncEnvironmentVariables() {
  return [
    ...commonTanaabEnvironmentVariables(),
...
export function buildAiSyncHelp(cli, environment = buildAiSyncEnvironment()) {
  return {
    description:
...
export function parseAiSyncArgs(argv, environment = buildAiSyncEnvironment()) {
...
export async function pruneDanglingSymlinks(rootPath) {
...
export async function runAiSync({ argv = process.argv.slice(2), cli }) {
  ...
  if (flags.help) {
    cli.showHelp(buildAiSyncHelp(cli), 0);
  }
