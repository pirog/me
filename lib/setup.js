...
function safeFailureReason(result) {
  ...
  const knownReasons = [
    [/\b(?:ENOTFOUND|getaddrinfo|DNS lookup failed)\b/i, 'network host could not be resolved'],
    [/\b(?:ECONNREFUSED|ETIMEDOUT|network is unreachable)\b/i, 'network connection failed'],
    [/\b(?:401|403|unauthorized|authentication required)\b/i, 'authentication was rejected'],
    [
      /\b(?:404|package .* not found|no matching package)\b/i,
      'requested package or resource was not found',
    ],
    [/\b(?:EACCES|EPERM|permission denied)\b/i, 'access was denied'],
    [
      /\b(?:lockfile.*(?:out of date|mismatch)|frozen lockfile)\b/i,
      'dependency lockfile is not satisfied',
    ],
    [/\b(?:no such file or directory|ENOENT)\b/i, 'a required file or executable is missing'],
  ];
  return (
    knownReasons.find(([pattern]) => pattern.test(output))?.[1] ??
    `process exited with status ${result.code}`
  );
}

async function required(command, args, options, operation = 'setup command') {
  const result = await run(command, args, options);
  if (result.code !== 0) {
    if (process.env.RUNNER_DEBUG === '1') {
      process.stderr.write(
        `debug: setup operation=${operation} reason=${safeFailureReason(result)}\n`,
      );
    }
    throw new BlockedError(`${command} failed (${result.code}).`);
  }
...
async function applyConfig({ root }) {
  await required('bun', ['run', 'ai:sync'], { cwd: root }, 'config.sync');
}
...
async function applyPlugin({ root, home, repo, name, optional, build }) {
  ...
  if (build) {
    await required('bun', ['run', 'build'], { cwd: source }, 'plugin.build');
    const stamp = path.join(source, 'node_modules', '.cache', 'me-setup-build.sha256');
  ...
  if (!(inspection?.installed && inspection.enabled === true)) {
    await required(cli, ['install', source], { cwd: source }, 'plugin.install');
  }
  await required(
    cli,
    ['cache', 'sync', '--repo-root', source],
    { cwd: source },
    'plugin.cache-sync',
  );
}
