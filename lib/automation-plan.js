...
function expectedTask(task, defaults, projects, targetThreadId) {
  const common = {
    name: task.name,
  ...
  if (task.kind === 'heartbeat') {
    return { ...common, kind: 'heartbeat', destination: 'thread', targetThreadId };
  }
...
function changedFields(actual, expected) {
  return EXPECTED_FIELDS.filter((field) => actual[field] !== (expected[field] ?? null));
}
 ...
 * @param {object} [options.threadTargets] Verified local task ids by manifest id for missing heartbeats.
...
export function buildAutomationPlan({
  ...
  if (
    ...
    !threadTargets ||
    typeof threadTargets !== 'object' ||
    Array.isArray(threadTargets)
  ) {
  ...
  for (const task of [...desiredTasks].sort((left, right) => left.id.localeCompare(right.id))) {
    ...
    if (actual && actual.kind !== (task.kind ?? 'cron')) {
      throw new Error(
        `automation ${task.id} needs an explicit kind migration; do not replace its task implicitly.`,
      );
    }
    const targetThreadId = actual?.targetThreadId ?? threadTargets[task.id] ?? null;
    if (task.kind === 'heartbeat') {
      if (
        targetThreadId !== null &&
        (typeof targetThreadId !== 'string' || !targetThreadId.trim())
      ) {
        throw new Error(`automation ${task.id} has an invalid target task id.`);
      }
      if (actual && threadTargets[task.id] && threadTargets[task.id] !== actual.targetThreadId) {
        throw new Error(`automation ${task.id} cannot silently change its target task.`);
      }
      if (!targetThreadId)
        blockers.push(
          `Resolve a local target task for ${task.id} through native task reads and supply threadTargets.${task.id}.`,
        );
    }
    const expected = expectedTask(task, defaults, projects, targetThreadId);
    if (!actual) {
