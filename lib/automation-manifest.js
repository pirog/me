...
async function normalizeTask(rawTask, index, repoRoot) {
  ...
  const kind = rawTask.kind ?? 'cron';
  if (!['cron', 'heartbeat'].includes(kind)) {
    throw new Error(`${label}.kind must be cron or heartbeat.`);
  }
  if (
    kind === 'heartbeat' &&
    ['local-project', 'model', 'reasoning'].some((key) => rawTask[key] !== undefined)
  ) {
    throw new Error(`${label}: heartbeat uses its target task's project and model settings.`);
  }
