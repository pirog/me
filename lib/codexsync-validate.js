...
import { loadAutomationManifest } from './automation-manifest.js';
import { loadModelRoutingPolicy } from './model-routing-policy.js';
import { validateSkillDir } from '../skills/skill-author/lib/skill-validator.js';
import parseFrontmatter from '../skills/skill-author/utils/parse-skill-frontmatter.js';
import formatSkillValidationReport from '../skills/skill-author/utils/format-skill-validation-report.js';
...
async function validateSkills(context) {
  ...
  for (const skillName of skillDirs) {
    ...
    context.stdout.write(`${formatSkillValidationReport(result)}\n`);
...
async function validateAutomations(context) {
  logCheck(context, 'automation manifest');
  try {
    await loadAutomationManifest({ repoRoot: context.repoRoot });
  } catch (error) {
    fail(context, `automation manifest validation failed: ${error.message}`);
  }
}
...
export async function runCodexSyncValidate(options = {}) {
  ...
  await validateAutomations(context);
  logCheck(context, 'model routing policy');
  try {
    await loadModelRoutingPolicy(context.repoRoot);
  } catch (error) {
    fail(context, `model routing policy validation failed: ${error.message}`);
  }
